...
In the co-training process, we have user set $\mathcal{A}$ and $\mathcal{B}$ as described in previous section. Let $l$ are $k$ be positive integers, and $k > l$. During each iteration, top $l$ users generated by the naive Bayes algorithm and top $l$ users generated by bidirectional snowball algorithm are merged into set $\mathcal{A}$. The merged users are considered as positive training examples during later training process. The iteration is executed until the top $k$ users generated by the bidirectional snowball algorithm and naive Bayes algorithm are the same. The result of the last execution of the naive Bayes algorithm is used as the final result of the co-training algorithm. We describe this algorithm in Table 
\ref{alg:cotrain}.

\begin{algorithm}[htbp]
\caption{\textsc{Co-training}}
\textbf{Input: }{Category $\mathcal{C}$, keyword $z_{\mathcal{C}}$}, parameter $k$ and $l$\\
\textbf{Output: }{An array $rank$ containing the users in $\mathcal{B}$ ranked on he probability of belonging to $\mathcal{C}$}
\begin{algorithmic}[1]
\REPEAT
\STATE Use $\mathcal{A}$ and $\mathcal{B}$ to perform bidirectional snowball algorithm.
\STATE Use $\mathcal{A}$ and $\mathcal{B}$ to perform naive Bayes algorithm.
\STATE Put top $l$ users returned by bidirectional snowball algorithm in $\mathcal{A}$.
\STATE Put top $l$ users returned by naive Bayes algorithm in $\mathcal{A}$.
\UNTIL{Top $k$ users returned by two algorithm are same.}
\RETURN $rank$ generated by naive Bayes algorithm.
\end{algorithmic}
\label{alg:cotrain}
\end{algorithm}
