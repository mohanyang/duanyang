...
or equivalently calculate the value of the following expression:
$$\log(p(c = 1)) - \log(p(c = 0)) + \sum_{j=1}^m (p(\phi_{u,j}|c = 1) - p(\phi_{u,j}|c = 0)).$$
The larger the value (positive value), the more likely $u$ belongs to $\mathcal{C}$; the smaller the value (negative value), the more likely $u$ does not belong to $\mathcal{C}$.
